...
const formatoMoneda = (valor) =>{ //Para dar formato de moneda
    return valor.toLocaleString('es-AR', {style: 'currency', currency: 'ARS', minimumFractionDigits:2}); 
}

const formatoPorcentaje = (valor) =>{ //Para dar formato al porcentaje
...
const eliminarIngreso = (id) =>{
    ...
    //for(let ingreso of ingresos) -> cumple una función similar a esto
...
let agregarDato = () =>{
    ...
    if(descripcion.value !== '' && valor.value !== ''){
        if(tipo.value === 'ingreso'){
            ingresos.push(new Ingreso(descripcion.value, +valor.value));//+ convierte una cadena en valor numérico
            cargarCabecero();
            cargarIngresos();
        }
